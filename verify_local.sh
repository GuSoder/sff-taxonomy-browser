...
# 2) const nodes array must JSON-decode and keep expected node/work counts.
set -e
cd "$(dirname "$0")"
python3 - <<'PY'
...
works=sum(len(n.get('works',[])) for n in nodes)
...
print(f'OK: script parses, {len(nodes)} nodes, {works} works, refs consistent')
PY
