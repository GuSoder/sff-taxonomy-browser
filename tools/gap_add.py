...
G = 'intake-lists/gap-2022'; TOTAL = 125
...
for it in json.load(open(sys.argv[1])):
    ...
    if a == 'add':
        ...
        prog['added'] += 1; log.write(f'{k}/{TOTAL} +{c["title"]} → {leaf["label"]}\n')
    elif a == 'merge':
...
print(f'awards {prog["awards_processed"]}/{TOTAL}, added {prog["added"]}, held {len(prog.get("held", []))}')
